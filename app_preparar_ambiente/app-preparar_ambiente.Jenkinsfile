pipeline {
  ...
  stages {
    stage('Build') {
      steps {
        sh 'docker build --file preparar-ambiente.Dockerfile --tag $DOCKERHUB_APP_PROJETO_ADA_IMAGE:preparar-ambiente .'
      }
    }
    stage('Deploy') {
      steps {
        withCredentials([usernamePassword(credentialsId: "${DOCKER_REGISTRY_CREDS}", passwordVariable: 'DOCKER_PASSWORD', usernameVariable: 'DOCKER_USERNAME')]) {
          ...
          sh 'docker push $DOCKERHUB_APP_PROJETO_ADA_IMAGE:preparar-ambiente'
        }
      }
    }
